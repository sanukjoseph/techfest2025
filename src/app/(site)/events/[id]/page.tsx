...
const EventDetailsPage = async ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  return (
    ...
    //         {data.image_url && <Image width={1000} height={1000} src={data.image_url ?? ""} alt={data.name ?? "Event Image"} className="w-fit h-3/5 border rounded-md object-contain shadow-lg" />}
    ...
    //           <CardHeader>
    //             <CardTitle className={`${squidgame.className} text-xl sm:text-2xl md:text-4xl font-bold text-foreground`}>{data.name} & Debugging</CardTitle>
    //           </CardHeader>
    //           <CardContent>
